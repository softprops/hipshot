  ...
  object rooms {
    ...
    case class Webhooks(room: String) {
      ...
      def create(
        ...
        event: String,
        name: String) =
        complete(webhookBase.POST << compact(render(
          ("url" -> url) ~
          ("pattern" -> pattern) ~
          ("event" -> event) ~
          ("name" -> name))))
    ...
    def topic(room: String, name: String) =
      complete(apiBase.PUT / room / "topic" << compact(render(
        "topic" -> name)))
  }
  ...
  object users {
    ...
    case class User(user: String) {
      ...
      def message(msg: String) =
        complete(thisUser.POST << compact(render(
          "message" -> msg)))
    ...
    case class Editor(
      ...
      def apply[T]
        (handler: Client.Handler[T])
        (implicit ec: ExecutionContext) =
        request(endpoint << compact(render(
              ("user_id"     -> userId) ~
              ("email"       -> _email) ~
              ("name"        -> _name) ~
